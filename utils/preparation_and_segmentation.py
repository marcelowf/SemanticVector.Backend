...
class PreparationAndSegmentation:
    # @staticmethod
    async def normalize_line_breaks(text: str) -> str:
        text = text.replace('\r\n', '\n').replace('\r', '\n')
        text = re.sub(r'\n{3,}', '\n\n', text)
        return text

    # @staticmethod
    async def split_sentences(text: str) -> list:
        sentences = []
    ...
    # @staticmethod
    async def build_sliding_windows(sentences: list, window_size: int = 4, overlap: int = 2) -> list:
        windows = []
